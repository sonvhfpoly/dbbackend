...
**`TaskInput.storage_url`** (nullable): public GCS URL khi input là 1 file thật, `null` khi chỉ là name/description tự khai (không có file đính kèm) — đáp ứng `STU-09 "Download Input"`. Tự động điền cho task tạo qua AI Task Builder: `TaskBuilderService.generate_task` copy toàn bộ `TBDocument` (tài liệu doanh nghiệp đính kèm lúc chat với AI) sang `TaskInput` của task vừa tạo (`input_type=DOCUMENT`), dùng lại chính `storage_url` public đã có ở `TBDocument` — xem [ARCHITECTURE.md](ARCHITECTURE.md) mục cross-domain `task_builder → task`. Task tạo thủ công (`POST /tasks/` không qua AI Task Builder) có thể tự khai `storage_url` nếu caller đã tự host file, hoặc để trống nếu input chỉ là mô tả text. Như `TaskSubmissionFile`, đây là public URL (không phải "Signed URL" như `STU-09` yêu cầu) — cùng đánh đổi MVP/demo đã áp dụng cho toàn bộ 2 bucket GCS của dự án.
...
**Public URL — lệch có chủ đích khỏi requirements.md §14** (§14 yêu cầu `private storage; signed URL; permission check`): file lưu ở bucket `SUBMISSION_FILES_GCS_BUCKET` riêng, được public ở mức bucket-IAM (`allUsers:objectViewer`) — `file_url` trả về là `https://storage.googleapis.com/...` xem được ngay, không cần auth/signed URL. Chọn vậy để đơn giản hoá demo/MVP (nhất quán với quyết định [no-auth](ARCHITECTURE.md#3-không-có-authrbac--quyết-định-có-chủ-đích) toàn dự án) — **không phải thiếu sót**, mà là đánh đổi đã cân nhắc, xác nhận và áp dụng luôn cho cả `TASK_BUILDER_GCS_BUCKET` (xem `TaskInput.storage_url` ở mục trên). Vẫn tách 2 bucket riêng vì `uniform-bucket-level-access` không thể public theo path/prefix trong cùng 1 bucket — không phải vì 1 bucket private/1 bucket public.

**Chưa làm** (biết trước, ngoài phạm vi hiện tại): MIME allow-list (§14 liệt kê DOCX/PDF/XLSX/PPTX/ZIP/PNG/JPG nhưng "Có thể hỗ trợ", không bắt buộc), virus scan thật (`NFR-16`, hiện là placeholder), và audit log `ActivityEvent`/`SUBMISSION_FILE_UPLOADED` (§17/§18 — hệ thống audit log tổng quát này chưa tồn tại ở bất kỳ domain nào).
...
### `EnterpriseReview` — phản hồi phía doanh nghiệp trên 1 submission ([requirements.md BUS-12](requirements.md#6-functional-requirements--business))

Cố tình tách biệt hoàn toàn khỏi `mentor_review`/`TaskSubmission.status`: BUS-12 định nghĩa `EnterpriseReview` như một bản ghi feedback độc lập với ràng buộc **"Không thay Evidence"** — tạo 1 review không bao giờ đổi `TaskSubmission.status` hay bất kỳ `EvidenceClaim` nào (`TaskService.create_enterprise_review` chỉ `INSERT`, không gọi `update_submission`). Append-only, nhiều review/submission (giống `TaskReview` ở Task).

| field | type | ghi chú |
|---|---|---|
| id | PK | |
| submission_id | FK → `TaskSubmission.id` | |
| reviewed_by | int, không FK | actor phía doanh nghiệp — tham chiếu lỏng, giống `reviewer_id`/`mentor_id` (không có auth) |
| decision | enum `EnterpriseReviewDecision` (ACCEPTED/CHANGES_REQUESTED) | khớp 2 nút "Chấp nhận"/"Yêu cầu chỉnh sửa" ở màn enterprise review |
| comment | str, nullable | |
| created_at | datetime | |

Nhúng sẵn trong `TaskSubmissionRead.enterprise_reviews` (giống cách `files` đã nhúng) — `GET /tasks/submissions/{id}` trả kèm luôn, không cần gọi riêng `GET .../enterprise-review`.
...
Business view (`GET /eportfolio/students/{id}/business-view`) trả `403` nếu chưa có consent — không có dữ liệu nào rò rỉ ra ngoài khi `share_with_business=False`. Đã qua được check này nghĩa là đã consent, nên response cũng nhúng luôn `career_suggestions` ([requirements.md §21](requirements.md#21-eportfolio-requirements) — "career interests nếu student share", dùng chung 1 flag consent, không có flag riêng cho career interests). `selected_evidence`/`selected_tasks` cũng có thêm `mentor_id` (int trần — không có Mentor identity model để resolve ra tên/rating, xem giả định bên dưới) và `joined_at`.

`GET /eportfolio/candidates` ([requirements.md §32](requirements.md#32-business-navigation) "Ứng viên"): liệt kê mọi student có `share_with_business=True`, trả tóm tắt (`full_name`, `headline`, `skill_count`, `evidence_count`) — không phải full portfolio, dùng cho màn hình danh sách trước khi bấm vào từng ứng viên.
...
| task | `EnterpriseReviewDecision` | ACCEPTED, CHANGES_REQUESTED |
...
- **Không có entity Mentor/User** — `reviewer_id`/`mentor_id`/`reviewed_by` (EnterpriseReview) chỉ là số nguyên, chưa gắn identity thật — kể cả ở `PortfolioEvidence.mentor_id` (business-view). Không thể resolve ra tên hay bất kỳ khái niệm "rating" nào (không tồn tại ở đâu trong codebase). Nếu cần audit theo từng mentor cụ thể hoặc hiển thị tên/đánh giá mentor, cần thêm domain identity (xem [ARCHITECTURE.md §3](ARCHITECTURE.md#3-không-có-authrbac--quyết-định-có-chủ-đích) — quyết định có chủ đích, chưa lên kế hoạch làm).
